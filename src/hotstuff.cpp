...
void HotStuffBase::vote_handler(MsgVote &&msg, const Net::conn_t &conn) {
    ...
    msg.postponed_parse(this);
    if(!msg.vote.cert) {
        // no vote handling, maybe add logic for pacemaker event
        HOTSTUFF_LOG_INFO("Got no-vote for %.10s", msg.vote.blk_hash.to_hex().c_str());
        return;
    }
    //auto &vote = msg.vote;
...
void HotStuffBase::req_blk_handler(MsgReqBlock &&msg, const Net::conn_t &conn) {
    for (const auto &h: msg.blk_hashes)
        HOTSTUFF_LOG_INFO("Got MsgReqBlock for %.10s", h.to_hex().c_str());
    ...
    promise::all(pms).then([replica, this](const promise::values_t values) {
        ...
        for (auto &v: values)
        {
            ...
            blks.push_back(blk);
            // logging for response block
            HOTSTUFF_LOG_INFO("Sending MsgRespBlock for %.10s", blk->get_hash().to_hex().c_str());
        }
