...
block_t HotStuffCore::on_propose(const std::vector<uint256_t> &cmds,
    ...
    // here starts block processing
    on_pre_process_propose(prop);
    on_deliver_blk(bnew);
    update(bnew);
    LOG_PROTO("propose %s", std::string(*bnew).c_str());
    // proposal creation finished. trigger propose waiting
    /* self-vote */
...
void HotStuffCore::on_receive_proposal(const Proposal &prop) {
    ...
    if (opinion && !vote_disabled && !is_in) // is_in was added, dont vote for certain heights for testing of failed consensus, for that store chosen heights as key in map with mapped value as index of replica, that shouldnt send a vote
    {
        ...
        do_vote(prop.proposer, Vote(id, bnew->get_hash(), nullptr, this));
    }
}
...
promise_t HotStuffCore::async_pre_process_proposal() {
    return pre_process_proposal_waiting.then([](const Proposal &prop) {
        return prop;
    });
}
...
void HotStuffCore::on_pre_process_propose(const Proposal &prop) {
    auto t = std::move(pre_process_proposal_waiting);
    pre_process_proposal_waiting = promise_t();
    t.resolve(prop);
}
